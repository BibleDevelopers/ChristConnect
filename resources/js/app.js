...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function populateChapters() {
        ...
        const chapters = parseInt(
            selectedOption.getAttribute("data-chapters"),
            10
        );
        clearAll(false);
        if (!chapters) return;
    ...
    function executeSearch() {
        ...
        loading.style.display = "block";
        let url = `/api/alkitab/search/${version}?q=${encodeURIComponent(
            query
        )}`;
    ...
    function renderContent(book, chapter, verses) {
        ...
        if (verses.length === 0) {
            html +=
                "<p>Data untuk pasal ini tidak tersedia dalam database sampel. Silakan coba Kejadian (Genesis) pasal 1 atau 2.</p>";
        } else {
            verses.forEach((v) => {
        ...
        if (results.length === 0) {
            ...
            results.forEach((r) => {
                const bookName = document.querySelector(
                    `#book-select option[value='${r.book}']`
                ).textContent;
                html += `
                    <div class="search-result-item">
                        <small>${bookName} ${r.chapter}:${
                    r.verse
                } (${r.version.toUpperCase()})</small>
                        <p>${r.text.replace(
